...
public class ImageProcess implements Runnable {
	...
	public MatOfPoint processImage(Mat img) {		
		...
		int minS = 150; // minimum saturation
		...
		Imgproc.findContours(_binary, contours, new Mat(), Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
		...
		for (int i = 0; i < nCheck; i++) {
			...
			if (cont.hullArea > 0 && cont.area / cont.hullArea > minDensity) { // if the hull area is more than 0 and the ratio is good
		...
		Log.d("time", Double.toString((System.nanoTime() - t) / 1000000.0));
